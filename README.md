...
The cache is also local to each Function worker. The dashboard shows statistics for the worker that serves the request, not an aggregate across scaled instances.

缓存同样仅存在于每个 Function worker 中。管理面板显示的是当前请求所在 worker 的统计信息，不是多个扩缩容实例的汇总。
